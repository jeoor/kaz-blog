...
export default function Pagination({ currentPage, totalPages, getPageHref, ariaLabel = "分页", className = "" }: Props) {
    ...
    const pageActive = "border-black/14 bg-black/[0.04] text-white dark:border-white/[0.08] dark:bg-white/[0.05] dark:text-white/94";
                        ...
                        if (item.type === "ellipsis") {
                            return (
                                <span
                                    key={item.key}
                                    aria-hidden="true"
                                    className="inline-flex h-10 min-w-10 items-center justify-center text-black/30 dark:text-white/46"
                                >
                                    …
                                </span>
                            );
